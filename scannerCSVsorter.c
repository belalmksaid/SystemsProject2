...
cell *get_cells(char **pre_cell, char data_type, int index, int len)
{
	...
	for (i = 0; i < len; i++)
	{
		...
		if (i == index)
		{
			...
			int st = 0;
			int end = 0;
			while (pre_cell[i][st] == ' ' || pre_cell[i][st] == '\t' || pre_cell[i][st] == '"')
				st++;
			while (pre_cell[i][end] != '\0')
				end++;
			end--;
			while (end > -1 && (pre_cell[i][end] == ' ' || pre_cell[i][end] == '\t' || pre_cell[i][end] == '"'))
				end--;
			if (end <= st)
				cells[i].is_empty = true;
			else
			{
...
int sort_file(char *file_path, char *dts, char *filename, char *header_to_sort, char *od)
{
	...
	if (cell_index != -1)
	{
		...
		FILE *fout;
		if (od == NULL)
		{ // od is null means that there is no specified output directory
